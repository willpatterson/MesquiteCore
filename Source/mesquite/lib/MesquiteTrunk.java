...
/**
 * The superclass of the Mesquite class, which resides within the trunk
 * MesquiteModule, has the "main" for the program Mesquite, and instantiates
 * itself as mesquiteTrunk. MesquiteTrunk is placed in the class library so that
 * all MesquiteModules (modules) can communicate with the trunk.
 */
public abstract class MesquiteTrunk extends MesquiteModule {
	public static boolean startedAsLibrary = false;
	// the start.Mesquite class that may have started Mesquite; to get class loaders
	// (Java 1.9 and above; 2O18)
	public static Object starter;
	// The URLClassLoader that is pointed to Mesquite_Folder/mesquite
	public static ClassLoader basicClassLoader;
	// turns on checking of classes in FileElement, NexusBlock and MesquiteCommand
	// (possibly others), to detect memory leaks
	...
	public static String[] standardPackages = new String[] { "minimal", "charMatrices", "basic", "trees", "categ",
			"cont", "ancstates", "charts", "lists", "io", "parsimony", "stochchar", "genesis" };
	/** List of standard packages. */
	public static String[] standardExtras = new String[] { "batchArch", "assoc", "correl", "coalesce", "diverse",
			"rhetenor", "align", "consensus", "treefarm", "molec", "search", "distance", "ornamental", "pairwise",
			"dmanager", "mb", "meristic", "tol" };
	...
	/**
	 * Default directory used in Open file dialogs and for consoles' current
	 * directory. Adjusted by minimal.BasicFileCoordinator
	 */
	...
	public static MesquiteMenuSpec fileMenu, editMenu, charactersMenu, treesMenu, analysisMenu, windowsMenu, helpMenu,
			utilitiesMenu;
	...
	public MesquiteCommand newFileCommand, openFileCommand, openURLCommand, showLicenseCommand, resetMenusCommand,
			currentCommandCommand, pendingCommandsCommand, forceQuitCommand, quitCommand, showAllCommand,
			closeAllCommand, saveAllCommand;
	...
	/**
	 * The panel in which all Mesquite "windows" fit if run as applet embedded in
	 * page.
	 */
	...
	/**
	 * Placed here so that MesquiteModules are able to request that the trunk module
	 * of Mesquite open a new file
	 */
	...
	public abstract MesquiteProject openOrImportFileHandler(String path, String completeArguments,
			Class importerSubclass); // TODO Use generics
	...
	@Override
	public Class getDutyClass() { // TODO deprecate and use generics
	...
	/**
	 * Returns the first three characters of "java.version" as a string; e.g.,
	 * "1.4.1" is returned as the string 1.4
	 */
	...
	/**
	 * Returns the full "java.version" as a string; e.g., "1.4.1" is returned as the
	 * string 1.4.1
	 */
	...
	/**
	 * Returns the first three characters of "java.version" as a double; e.g.,
	 * "1.4.1" is returned as the double 1.4
	 */
	...
	/**
	 * Updates the menu items to ensure that they are enabled or disabled as
	 * appropriate.
	 */
	public final static void resetMenuItemEnabling() {
		try {
			Enumeration e = mesquiteTrunk.windowVector.elements();
			while (e.hasMoreElements()) { // TODO use generics
				...
				Vector v = mw.getOwnerModule().embeddedMenusVector; // TODO use generics
				if (v != null) {
		...
		} catch (Exception e) { // TODO catch and handle exceptions
		}
	}
	...
	/**
	 * Updates the checked menu items to ensure that they are checked or not as
	 * appropriate.
	 */
	public final static void checkForResetCheckMenuItems() {
		...
		if (resetCheckSuppressed)
			return;
		try {
			Enumeration e = mesquiteTrunk.windowVector.elements(); // TODO use generics
	...
	/** Simply sets a flag that is dealt with on the ClockWatcherThread */
	...
	private static void resetChecks(Menu menu, MesquiteMenuSpec menuSpec) {
		...
		for (int i = 0; i < numItems; i++) {
			...
			else if (mi instanceof MesquiteCheckMenuItem && menu instanceof MesquiteSubmenu
					&& menuSpec == ((MesquiteSubmenu) menu).getSpecification()) {
				((MesquiteCheckMenuItem) mi).resetCheck();
			}
		}
	}
